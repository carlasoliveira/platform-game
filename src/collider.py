...
class Collider:
    ...
    def check_collider(self, static_object, dynamic_object):
        ...
        distance = pg.math.Vector2(
            dynamic_pos.x - static_pos.x, dynamic_pos.y - static_pos.y)
        ...
        collision_vector.x = math.copysign(
            target_distance.x - abs(distance.x), distance.x)

        collision_y = abs(distance.y) < target_distance.y
        collision_vector.y = math.copysign(
            target_distance.y - abs(distance.y), distance.y)
        ...
        for platform in platforms:
            ...
            if colliding:
                ...
                if abs_ds_y <= abs_ds_x:
                    dyn_pos.y += ds.y
                    dynamic_object.set_velocity(pg.math.Vector2(
                        dynamic_object.get_velocity().x, 0))
                    ...
                    dyn_pos.x += ds.x
                    dynamic_object.set_velocity(pg.math.Vector2(
                        0, dynamic_object.get_velocity().y))
        ...
        for puzzle in puzzles:
            ...
            if colliding:
                if puzzle.get_puzzle_type() == "movable_block":
                    self._handle_movable_block_collision(
                        player, puzzle, ds, abs_ds_x, abs_ds_y)
    ...
    def _handle_door_collision(self, player, door, ds, abs_ds_x, abs_ds_y):
        if not door.is_activated():
            ...
            if abs_ds_y <= abs_ds_x:
                player_pos.y += ds.y
                player.set_velocity(pg.math.Vector2(
                    player.get_velocity().x, 0))
                if ds.y < 0:
                    player.m_is_on_ground = True
            else:
                player_pos.x += ds.x
                player.set_velocity(pg.math.Vector2(
                    0, player.get_velocity().y))
    ...
    def resolve_puzzle_lava_collision (self, puzzles, lavas):
        for puzzle in puzzles:
            if puzzle.can_be_pushed():
                self.resolve_collision(puzzle, lavas)
                    ...
                    if colliding:
                        if abs_ds_y <= abs_ds_x:
                            ...
                            if ds.x < 0:
                                ...
                                puzzle_a.set_position(puzzle_a_pos)
                                puzzle_a.set_velocity(pg.math.Vector2(
                                    0, puzzle_a.get_velocity().y))
                            else:
                                puzzle_b_pos = puzzle_b.get_position()
                                puzzle_b_pos.x += ds.x
                                puzzle_b.set_position(puzzle_b_pos)
                                puzzle_b.set_velocity(pg.math.Vector2(
                                    0, puzzle_b.get_velocity().y))
